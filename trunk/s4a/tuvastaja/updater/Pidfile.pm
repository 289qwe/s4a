...
sub _get_pid
{
    ...
    if (!open( PID, $pidfile )) {
	 syslog(LOG_ERR, "can't read pid file $pidfile");
...
sub _create_pidfile
{
    ...
    if ( -e $pidfile )
    {
        ...
        if ( _is_running( $pid ) )
        {
            ...
            {
                syslog(LOG_WARNING,"$0 already running: $pid ($pidfile)");
        ...
        else
        {
            ...
            if (!open( PID, ">$pidfile" )) {
                syslog(LOG_WARNING, "Can't write to $pidfile");
...
sub _destroy_pidfile
{
    ...
    {
        syslog(LOG_ERR, "pidfile $pidfile doesn't exist");
    ...
    else
    {
        ...
        if ( $ppid != $pid )
        {
            syslog(LOG_WARNING, "pid $pid in $pidfile is not mine ($$) - I am $0 - or my parents ($ppid)");
